...
function rawFieldValue(value, field) {
    if (field.valueType === 'stringArray') {
        return Array.isArray(value) ? value.join(', ') : '';
    }
    if (field.valueType === 'boolean') {
...
function cycleEnum(field, current, direction) {
...
export async function runConfigTui(configPath, defaultConfigTemplate, resolveConfigPath) {
    ...
    const renderList = () => {
        ...
        const width = Math.max(20, keysList.width);
    ...
    const cycleSelectedEnum = (direction) => {
