...
export function createCallbackServer({ callbackHost, callbackPort, callbackAuthToken, callbackMaxBodyBytes, messagingPlatform, cronScheduler, messagingClient, getLastIncomingChatId, semanticConversationMemory, conversationHistoryMaxTotalChars, conversationHistoryRecapTopK, logInfo, }) {
    let callbackServer = null;
    const platformCallbackPath = `/callback/${messagingPlatform}`;
    const sendLoggedJson = (req, res, statusCode, payload) => {
    ...
    const handleCallbackRequest = async (req, res) => {
        ...
        if (requestUrl.pathname === '/api/memory/semantic-recall') {
            ...
            const input = body.input.filter((item) => typeof item === 'string').map((s) => s.trim());
            ...
            const topK = Number.isFinite(topKRaw) && topKRaw > 0 ? Math.max(1, Math.floor(topKRaw)) : conversationHistoryRecapTopK;
            if (input.length === 0) {
        ...
        const isCallbackEndpoint = requestUrl.pathname === '/callback' ||
        ...
        const targetChatId = resolveChatId(body?.chatId ?? requestUrl.searchParams.get('chatId') ?? getLastIncomingChatId());
        if (!targetChatId) {
            sendLoggedJson(req, res, 400, {
                ok: false,
