...
class StreamingMessageSender {
    ...
    finalized = false;
    debouncedFlush;
    constructor(messageContext, requestId, maxResponseLength, streamUpdateIntervalMs, logInfo, getErrorMessage, acpDebugStream) {
        this.messageContext = messageContext;
        this.requestId = requestId;
        this.maxResponseLength = maxResponseLength;
        this.logInfo = logInfo;
        this.getErrorMessage = getErrorMessage;
        this.acpDebugStream = acpDebugStream;
        this.debouncedFlush = debounce(async () => {
            await this.sendNewContent();
        }, streamUpdateIntervalMs, { leading: false, trailing: true });
    }
    append(chunk) {
        this.buffer += chunk;
...
export async function processSingleTelegramMessage(params) {
    const { messageContext, messageRequestId, maxResponseLength, streamUpdateIntervalMs, acpDebugStream, approvalMode, maxRetries = 3, retryDelayMs = 5000, runAcpPrompt, scheduleAsyncJob, logInfo, getErrorMessage, onConversationComplete, } = params;
    ...
    const streamSender = new StreamingMessageSender(messageContext, messageRequestId, maxResponseLength, streamUpdateIntervalMs, logInfo, getErrorMessage, acpDebugStream);
    if (!isHybridMode) {
            ...
            try {
                ...
                fullResponse = await runAcpPrompt(prompt, async (chunk) => {
                    ...
                    if (conversationMode === ConversationMode.ASYNC)
                        return; // Suppress output for async
                ...
                const isRetriable = errorMessage.toLowerCase().includes('capacity') ||
