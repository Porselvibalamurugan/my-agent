...
describe('messageTruncator', () => {
    ...
    describe('splitIntoSmartChunks', () => {
        ...
        it('should split a long error stack trace inside a code block', () => {
            const lines = Array.from({ length: 20 }, (_, i) => `    at Module${i}.execute (src/module${i}.ts:${10 + i}:${5 + i})`);
        ...
        it('should split a message with MarkdownV2 special characters in prose', () => {
            const text = 'Results (3 of 10): score = 95.5% | status: pass.\n\n' +
        ...
        it('should split a message with inline code containing special chars', () => {
            const text = 'Run the command `curl -X POST https://api.example.com/v1/users -H "Authorization: Bearer $TOKEN"` ' +
            ...
            expect(chunks.some((c) => c.includes('`curl -X POST https://api.example.com/v1/users -H "Authorization: Bearer $TOKEN"`'))).toBe(true);
        ...
        it('should preserve all text content when splitting (no data loss)', () => {
            const text = 'Line one of the message.\nLine two has more content here.\n\n' +
        ...
        it('should split a message with nested markdown: bold inside link text', () => {
            const text = 'Check the report: [**Critical** findings](https://security.example.com/report/2024) ' +
            ...
            expect(chunks.some((c) => c.includes('[**Critical** findings](https://security.example.com/report/2024)'))).toBe(true);
            expect(chunks.some((c) => c.includes('[**High** priority items](https://security.example.com/report/high)'))).toBe(true);
        });
