...
export function registerMessagingHandlers({ messagingClient, telegramWhitelist, enforceWhitelist = true, platformLabel = 'Messaging', hasActiveAcpPrompt, cancelActiveAcpPrompt, cancelAllJobs, shutdownAgent, resetAgent, enqueueMessage, onAbortRequested, onChatBound, logError, logWarn, }) {
    const handleIncomingTelegramMessage = async (messageContext) => {
        ...
        if (enforceWhitelist && !isAuthorized) {
            logWarn(`Unauthorized access attempt from username: ${messageContext.username ?? 'none'} (ID: ${messageContext.userId ?? 'unknown'})`);
        ...
        enqueueMessage(messageContext).catch(async (error) => {
    ...
    const handleTelegramClientError = (error, messageContext) => {
