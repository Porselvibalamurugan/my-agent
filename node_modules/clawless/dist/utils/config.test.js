...
describe('config utils', () => {
    ...
    describe('getConfig', () => {
        ...
        it('validates Telegram platform requires token', () => {
            ...
            expect(logErrorSpy).toHaveBeenCalledWith(expect.stringContaining('TELEGRAM_TOKEN environment variable is required'));
        });
        it('validates Slack platform requires tokens', () => {
            ...
            const logErrorSpy = vi.spyOn(errorUtils, 'logError').mockImplementation(() => { });
            expect(() => getConfig()).toThrow('process.exit called');
            expect(logErrorSpy).toHaveBeenCalledWith(expect.stringContaining('SLACK_BOT_TOKEN environment variable is required'));
        });
    });
});
