 ...
 */
function terminateProcessGracefully(childProcess, agentDisplayName, killGraceMs, logInfo, details) {
 ...
 */
export async function runPromptWithCli(options) {
    const { scheduleId, promptForAgent, cliAgent, cwd, timeoutMs, noOutputTimeoutMs, progressIntervalMs = 60_000, logInfo, logError, onProgress, } = options;
    ...
    return new Promise((resolve, reject) => {
        ...
        const progressInterval = setInterval(async () => {
            ...
            if (noOutputTimeoutMs && idleMs >= noOutputTimeoutMs && !noOutputTimedOut) {
                ...
                await terminateProcessGracefully(tempProcess, agentDisplayName, killGraceMs, logInfo, { scheduleId });
                reject(new Error(`Scheduler ${agentDisplayName} killed: ${msg}`));
                return;
            }
            emitProgress('running', `⏳ ${agentDisplayName} working… (${Math.round((now - startTime) / 1000)}s elapsed, ${stdoutBytes} bytes received)`);
        }, progressIntervalMs);
        const cleanup = () => {
